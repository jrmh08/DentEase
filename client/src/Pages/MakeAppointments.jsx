...
function MakeAppointments() {
  ...
  const handleSubmit = async () => {
    ...
    if (!gapiInited || !gisInited) {
      setSnackbarOpen(true);
      setSnackbarMessage('Google API not initialized. Please try again later.');
      return;
    }
  ...
  const createCalendarEvent = async (appointmentData) => {
    const event = {
      'summary': `Dental Appointment - ${appointmentData.service}`,
      'location': 'DentEase Clinic',
      'description': appointmentData.note,
      'start': {
        'dateTime': `${appointmentData.date.format('YYYY-MM-DD')}T${appointmentData.time.format('HH:mm:ss')}`,
        'timeZone': 'Asia/Manila'
      },
      'end': {
        'dateTime': `${appointmentData.date.format('YYYY-MM-DD')}T${appointmentData.time.add(1, 'hour').format('HH:mm:ss')}`,
        'timeZone': 'Asia/Manila'
      },
      // Add attendees here
      'attendees': [
        { 'email': appointmentData.email } // Use the email submitted
      ],
    };
    ...
    try {
      ...
      setSnackbarOpen(true);
      setSnackbarMessage('Appointment submitted and added to Google Calendar.');
    } catch (error) {
      console.error('Error creating Google Calendar event:', error);
      setSnackbarOpen(true);
      setSnackbarMessage('Error adding event to Google Calendar.');
    }
  };
  ...
  const submitAppointment = async (appointmentData) => {
    ...
    try {
      ...
      setFormValid(true);
      setSnackbarOpen(true);
      setSnackbarMessage('Form submitted successfully and added to Google Calendar.');
    } catch (error) {
      console.error('Error submitting appointment:', error);
      setSnackbarOpen(true);
      setSnackbarMessage('Error submitting appointment. Please try again later.');
          ...
          <Box sx={{ width: '500px' }}>
            ...
            <Grid container columnSpacing={{ xs: '10px'}}>
              ...
              <Grid item>
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DemoContainer components={['MobileDatePicker']}>
                    <FormControl sx={{ m: 1, minWidth: 120, bgcolor: 'white' }} error={errors.date}>
                      <MobileDatePicker
                        className="gridpicker"
                        label="Select Date"
                        onChange={(date) => handleDateTimeChange(date, 'date')}
                        value={appointment.date}
                        textField={({ inputRef, ...others }) => (
                          <TextField {...others} InputLabelProps={{ shrink: true }} inputRef={inputRef} />
                        )}
                      />
                      {errors.date && <FormHelperText>No Appointments on Sundays</FormHelperText>}
                    </FormControl>
              ...
              <Grid item>
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DemoContainer components={['MobileTimePicker']}>
                    <FormControl sx={{ m: 1, minWidth: 120, bgcolor: 'white' }} error={errors.time}>
                          <MobileTimePicker
                          label="Select Time"
                          onChange={(time) => handleDateTimeChange(time, 'time')}
                          value={appointment.time}
