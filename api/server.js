...
app.get('/checkAppointmentStatus/:appointmentId', async (req, res) => {
  const { appointmentId } = req.params;
  
  db.query('SELECT * FROM `appointment_table` WHERE appointment_id = ?', [appointmentId], async (err, result) => {
    if (err) {
      console.error('Error checking appointment status:', err);
      res.status(500).send('Error checking appointment status');
    } else if (result.length > 0) {
      ...
      if (appointment.status === 'approved') {
        ...
        try {
          const event = {
            'summary': `Dental Appointment - ${appointment.service}`,
            ...
            'end': {
              'dateTime': `${appointment.date}T${appointment.time.split(':')[0]}:${(parseInt(appointment.time.split(':')[1]) + 60) % 60}:00`,
              'timeZone': 'Asia/Manila'
            },
          };

          const response = await createCalendarEvent(event);
          res.status(200).json({ message: 'Appointment approved and added to calendar', eventLink: response.htmlLink });
        } catch (error) {
          console.error('Error creating calendar event:', error);
          res.status(500).send('Error creating calendar event');
        }
      } else {
        res.status(200).json({ message: 'Appointment not yet approved' });
      }
    } else {
...
// Start the server
app.listen(port, () => {
  console.log(`DentEase app listening on port ${port}`);
});